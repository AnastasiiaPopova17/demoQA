...
public class PracticeFormPage extends BasePage{
    ...
    public PracticeFormPage enterBirthDay(String dataBirth) {
        clickWithJSScroll(inputDataOfBirth,0,300);
        Keys key = getKeyForOSCommandControl();
        inputDataOfBirth.sendKeys(key,"a");
        inputDataOfBirth.sendKeys(dataBirth);
        inputDataOfBirth.sendKeys(Keys.ENTER);
        return this;
    }
        ...
        for(String str: strings) {
            for(WebElement element: allHobbies) {
                if(getTextBase(element).equals(str)){
                    clickWithJSScroll(element,0,300);
                    break;
                }
            }
        }
...
//        clickBase(inputCity);
    ...
    public PracticeFormPage clickOnSubmit() {
        clickWithJSScroll(btnSubmit,0,300);
        return this;
    }
